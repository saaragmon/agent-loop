...
_ORCHESTRATOR_RETRIES = 3  # Gemini 5xx on the first call is common; retry before wrapping up empty
...
async def _run_once(team: Team) -> None:
...
async def _run(team: Team) -> None:
    for attempt in range(1, _ORCHESTRATOR_RETRIES + 1):
        try:
            await _run_once(team)
            return
            ...
            # A 5xx from Gemini on the orchestrator's own call. If nothing has launched yet,
            # retry a few times; otherwise stop cleanly and keep whatever is already on disk.
            if not team.objectives and attempt < _ORCHESTRATOR_RETRIES:
                print(f"\n  NOTE: Gemini returned {type(exc).__name__} ({exc}); retrying ({attempt}/{_ORCHESTRATOR_RETRIES})...")
                await asyncio.sleep(2 * attempt)
                continue
            print(
                f"\n  NOTE: the orchestrator stopped early after an error ({type(exc).__name__}: {exc}); "
                "wrapping up with what is built."
            )
            return
        except Exception as exc:
            # Any other model or network fault would otherwise crash the run with a traceback.
            print(
                f"\n  NOTE: the orchestrator stopped early after an error ({type(exc).__name__}: {exc}); "
                "wrapping up with what is built."
            )
...
def _ensure_site(team: Team) -> None:
    ...
    orchestrator stopped before authoring them. Rewrites the hub if it is missing or
    does not yet link every finished game (a leftover empty index.html from an earlier
    failed run would otherwise leave the arcade with no doors)."""
    if not (team.site_dir / "common.css").exists():
        css_agent.write_template_style(team.site_dir)
    games = team.finished_games()
    index = team.site_dir / "index.html"
    html = index.read_text(encoding="utf-8") if index.exists() else ""
    missing_links = [g for g in games if f'{g["slug"]}/game.html' not in html]
    if not index.exists() or missing_links:
        css_agent.write_template_hub(team.language, games, team.site_dir)
